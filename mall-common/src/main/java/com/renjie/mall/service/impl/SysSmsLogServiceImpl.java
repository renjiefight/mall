...
import com.renjie.mall.dao.SysSmsLogDao;
import com.renjie.mall.entity.SmsConfig;
import com.renjie.mall.entity.SysSmsLogEntity;
import com.renjie.mall.service.SysConfigService;
import com.renjie.mall.service.SysSmsLogService;
import com.renjie.mall.utils.*;
...
public class SysSmsLogServiceImpl implements SysSmsLogService {
    ...
    public SysSmsLogEntity sendSms(SysSmsLogEntity smsLog) {
        ...
        if (StringUtilTools.isNullOrEmpty(config)) {
            throw new RRException("请先配置短信平台信息");
        }
        if (StringUtilTools.isNullOrEmpty(config.getAppid())) {
            throw new RRException("请先配置短信平台APPID");
        }
        if (StringUtilTools.isNullOrEmpty(config.getAppkey())) {
            throw new RRException("请先配置短信平台KEY");
        }
        if (StringUtilTools.isNullOrEmpty(config.getSign())) {
