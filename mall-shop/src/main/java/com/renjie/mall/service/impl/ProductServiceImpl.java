...
import com.renjie.mall.entity.ProductEntity;
import com.renjie.mall.service.ProductService;
import com.renjie.mall.utils.BeanUtils;
...
public class ProductServiceImpl implements ProductService {
    ...
    public List<ProductEntity> queryList(Map<String, Object> map) {
        ...
        if (null != list && list.size() > 0) {
            for (ProductEntity item : list) {
                ...
                if (!StringUtilTools.isNullOrEmpty(specificationIds)) {
    ...
    public int save(ProductEntity product) {
        ...
        if (!StringUtilTools.isNullOrEmpty(goodsSpecificationIds)) {
            ...
            for (int i = 0; i < goodsSpecificationIdArr.length - 1; i++) {
                ...
                for (int j = 0; j < oneId.length; j++) {
                    for (int k = 0; k < twoId.length; k++) {
                        ...
                        if (StringUtilTools.isNullOrEmpty(oneId[j]) || StringUtilTools.isNullOrEmpty(twoId[k])){
                            continue;
                        }
    ...
    public int update(ProductEntity product) {
        if (StringUtilTools.isNullOrEmpty(product.getGoodsSpecificationIds())){
